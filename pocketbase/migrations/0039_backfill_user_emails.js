migrate(
  (app) => {
    ...
    try {
      ...
      console.log('0039: error fetching users: ' + e.message)
      return
    }
    ...
    for (var j = 0; j < allUsers.length; j++) {
      ...
      if (hasValidEmail) {
        try {
          ...
          console.log('0039: save failed for user ' + user.id + ' (has email): ' + e.message)
          errors++
        }
        continue
      }
      ...
      try {
        ...
        console.log('0039: failed to save user ' + user.id + ': ' + err.message)
        errors++
      }
    }
    ...
    var withoutEmail = 0
    try {
      verifyUsers = app.findRecordsByFilter('_pb_users_auth_', "id != ''", 'created', 0, 0)
      for (var v = 0; v < verifyUsers.length; v++) {
        var ve = verifyUsers[v].getString('email')
        if (ve && ve.trim() !== '') {
          withEmail++
        } else {
          withoutEmail++
      ...
      console.log('0039: verification error: ' + e.message)
    }

    console.log(
      '0039_backfill_user_emails: updated=' +
        updated +
        ' skipped=' +
        skipped +
        ' errors=' +
    ...
    console.log('0039_verify: with_email=' + withEmail + ' without_email=' + withoutEmail)
  },
  (app) => {},
)
