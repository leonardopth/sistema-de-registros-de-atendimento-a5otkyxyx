migrate(
  (app) => {
    ...
    function focUser(name, email, role, sg, bases, approvalStatus) {
      try {
    ...
    function focAE(name, email, phone, bases) {
      try {
    ...
    function focClient(data) {
      try {
        ...
        if (data.aeId) rec.set('account_executive_rel', data.aeId)
        rec.set('avoidable_contact_threshold', data.threshold)
        if (data.blocked) {
          rec.set('blocked', true)
          rec.set('block_reason', data.blockReason)
          if (data.blockedById) rec.set('blocked_by', data.blockedById)
    ...
    function focAgent(name, email, phone, clientId) {
      try {
    ...
    function srExists(description) {
      try {
        app.findFirstRecordByData('service_records', 'description', description)
        return true
      } catch (_) {
        return false
      }
    ...
    function shareExists(srId, userId) {
      try {
    ...
    if (!adminUser.get('master_access')) {
    ...
    for (var i = 0; i < userData.length; i++) {
      var u = userData[i]
      var rec = focUser(u.name, u.email, u.role, u.sg, u.bases, 'Aprovado')
      userMap[rec.id] = rec
    }
    ...
    if (consultantUsers.length === 0) return
    ...
    for (var ai = 0; ai < aeData.length; ai++) {
      var ae = aeData[ai]
      aeMap[ae.name] = focAE(ae.name, ae.email, ae.phone, ae.bases)
    }
    ...
    for (var ag = 0; ag < agentData.length; ag++) {
      ...
      var agRec = focAgent(ad.name, ad.email, ad.phone, clientRec.id)
    ...
    for (var si = 0; si < descriptions.length; si++) {
      if (srExists(descriptions[si])) {
        try {
          var existingSr = app.findFirstRecordByData(
            'service_records',
            'description',
            descriptions[si],
          )
          srIds.push(existingSr.id)
        } catch (_) {}
        continue
      }
      ...
      if (aeRec) {
        srRec.set('account_executive', aeRec.id)
      }
      ...
      if (isEmAndamento) {
        ...
        srRec.set('timer_running', false)
      }
      if (isConcluido && (si === 6 || si === 14)) {
        srRec.set(
    ...
    for (var fi = 0; fi < feedbackData.length; fi++) {
      ...
      try {
        app.findFirstRecordByData('feedback', 'message', fd.message)
      } catch (_) {
    ...
    for (var ni = 0; ni < notifData.length; ni++) {
      var nd = notifData[ni]
      try {
        app.findFirstRecordByData('notifications', 'title', nd.title)
      } catch (_) {
    ...
    for (var shi = 0; shi < shareData.length; shi++) {
      ...
      if (shareExists(srIds[sh.srIdx], sh.userId)) continue
    ...
    var auditCount = 0
    try {
      auditCount = app.countRecords('audit_log')
    } catch (_) {}
    ...
    if (auditCount < 5) {
      var auditData = [
        ...
        },
        {
          ...
          action: 'block',
          entity: 'clients',
          entityId: clientMap['Expresso Andes Viagens']
            ? clientMap['Expresso Andes Viagens'].id
            : '',
