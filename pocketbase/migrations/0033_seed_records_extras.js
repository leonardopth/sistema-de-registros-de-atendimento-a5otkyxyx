migrate(
  (app) => {
    ...
    var allUsers = app.findRecordsByFilter(
      'users',
      ...
      try {
        app
          ...
          .bind({
            ...
            created: createdTs,
            updated: createdTs,
            client_name: clientNames[clientIdx],
            client_email: clientEmails[clientIdx],
            client_phone: clientPhones[clientIdx],
            client_company: clientCompanies[clientIdx],
            ...
            duration: duration,
            end_time: endTime,
            assigned_agent: agentNames[agentIdx],
            user_id: userIds[userIdx],
            assigned_user: userIds[userIdx],
    ...
    for (var hi = 0; hi < historyEntries.length; hi++) {
      ...
      var histRec = new Record(historyCol)
    ...
    for (var nci = 0; nci < clientIds.length; nci++) {
      ...
      try {
        var avoidableCount = app.findRecordsByFilter(
          'service_records',
          "avoidable_contact = true && client = '" + clientIds[nci] + "'",
          '-created',
          0,
          0,
        ).length
        ...
        if (avoidableCount >= threshold) {
          ...
          var aeUserIdx = -1
          for (var aui = 0; aui < allUsers.length; aui++) {
            if (allUsers[aui].getString('email') === aeEmails[nci]) {
              aeUserIdx = aui
              break
            }
          }
          if (aeUserIdx >= 0) {
            ...
            aeNotif.set('user_id', userIds[aeUserIdx])
